...
utils.rackspace = function () {
  ...
  function makeRequest (target) {
    ...
    request(serverOptions, function (err, res, body) {
      if (err) {
        if (tries++ < client.config.retries) {
          console.warn('cloudfiles.rackspace: retrying '+target);
...
utils.rackspaceRequest = function (options, callback, success, tries) {
  if (!tries) tries = 0;
  ...
  function makeRequest() {
    ...
    // set enough max sockets on this target host
    httpClient.getAgent(httpOptions.host, httpOptions.port).maxSockets = 100;
    ...
    req.on('error', function(error) {
      ...
      if (tries++ < client.config.retries) {
        console.warn('cloudfiles.rackspaceRequest error: '+error+', retrying request to '+options.uri);
        return utils.rackspaceRequest(options, callback, success, tries);
      } else
