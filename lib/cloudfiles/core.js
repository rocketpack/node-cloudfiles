...
Cloudfiles.prototype.setAuth = function (callback) {
  ...
  var tries = 0;
  function authenticate() { // authenticate to CloudFiles, retry 3 times
    request(authOptions, function (err, res, body) {
      if (err) {
          if (tries++ < this.config.retries) {
            console.warn('cloudfiles.setAuth: retrying '+authOptions.uri);
            return authenticate();
          }
  ...
  var self = this;
  utils.statOrMkdir(this.config.cache.path, authenticate);
};
...
Cloudfiles.prototype.addFile = function (container, file, local, options, callback) {
